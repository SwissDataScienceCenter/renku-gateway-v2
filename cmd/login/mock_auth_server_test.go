...
func (t *testAuthServer) tokenEndpoint(c echo.Context) error {
	if t.Authorized {
		res := oauth2.Token{AccessToken: t.AccessToken, RefreshToken: t.RefreshToken}
		res = *res.WithExtra(map[string]string{"id_token": "idToken"})
		idToken, err := t.getJWT()
		if err != nil {
			return err
		}
