...
type Session struct {
	...
	LoginURLs          SerializableStringSlice
...
func (s *Session) PopLoginURL() string {
	if len(s.LoginURLs) == 0 {
		return ""
	}
	output := s.LoginURLs[0]
	s.LoginURLs = append(SerializableStringSlice{}, s.LoginURLs[1:]...)
	return output
}

func (s *Session) SetLoginURLs(urls []string) {
	s.LoginURLs = urls
}
